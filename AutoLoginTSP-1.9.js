...
const targetGitlabENSE = "https://gitlabense.imtbs-tsp.eu/users/sign_in";        // GitLabENSE
...
const targetDeclicMinet = "declic.minet.net";                                    // Stockage Déclic
...
// Stratégie d'autologin de GitlabENS
function stratGitlabENS(){
    try{ document.querySelector('button[type="submit"] img[alt="Shibboleth"]').click();} catch(e) {}
}

// Stratégie d'autologin de GitlabENS
function stratGitlabENSE(){
    try{ document.querySelector('button[data-testid="saml-login-button"]').click();} catch(e) {}
}
...
function stratDeclicMinet(){
    try{ document.querySelector('input.button.button--block').click();} catch(e) {}
}
...
(function() {
    ...
    // GitlabENS
        ...
        console.log("[AutoLoginTSP] Connexion à GitlabENS");

        // Applique la stratégie d'autologin de Gitlab
        stratGitlabENS();

        console.log("[AutoLoginTSP] Stratégie de GitlabENS appliquée");
    }

    // GitlabENSE
    else if (currentURL == targetGitlabENSE) {
        console.log("[AutoLoginTSP] Connexion à GitlabENSE");

        // Applique la stratégie d'autologin de GitlabENSE
        stratGitlabENSE();

        console.log("[AutoLoginTSP] Stratégie de GitlabENSE appliquée");
    }
    ...
    // Stockage Déclic
    else if ((currentURL.includes(targetDeclicMinet)) && idAutocomplete){
        console.log("[AutoLoginTSP] Connexion à Déclic");

        // Applique la stratégie d'autologin des Impressions
        stratDeclicMinet();

        console.log("[AutoLoginTSP] Stratégie de Déclic appliquée");
    }
